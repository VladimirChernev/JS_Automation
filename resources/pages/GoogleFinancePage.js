...
class GoogleFinancePage extends Page {
    ...
    async monitorBtcToUsdExchangeRateUi(period, interval) {
        ...
        for (let value of collected_values) {
            this.world.monitoredValues.push(parseFloat(value.replace(/,/g, '')))
        }

        if(this.world.debug) console.log("Collected values: " + this.world.monitoredValues);
    }
    ...
    async monitorBtcToUsdExchangeRateApi(period, interval) {
        ...
        // collect all values for the required period from the element in the html response:
        const collected_values = []
        for (let step = 0; step < Math.abs(period * 60 / interval) + 1; step++) {
    ...
    async checkOverallDifference(percent) {
        ...
        // get first value
        const initialValue = this.world.monitoredValues.at(0)
        // get last value
        const finalValue = this.world.monitoredValues.at(this.world.monitoredValues.length - 1)
    ...
    async checkIntervalsDifference(percent) {
        ...
        const highestNumber = Math.max(...this.world.monitoredValues);
        // get the lowest number
        const lowestNumber = Math.min(...this.world.monitoredValues);
        // calculate percentage difference
